...
def save_nifti(tensor, path, affine=None):
    ...
    if affine is None:
        # EXPORT_AFFINE_1P5MM matches the voxel order produced by the pipeline,
        # so saved volumes carry the correct left/right labelling. Copy it before
        # editing so the module-level constant is never mutated.
        affine = np.array(getattr(const, 'EXPORT_AFFINE_1P5MM', np.eye(4) * 1.5),
