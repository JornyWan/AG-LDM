...
# ---------------- Utils ----------------
def save_nifti_array(arr, path, affine=const.EXPORT_AFFINE_1P5MM):
    # EXPORT_AFFINE_1P5MM matches the voxel order the pipeline actually produces,
    # so exported volumes carry the correct left/right labelling. Reported metrics
    # are unaffected: image and tissue metrics are computed on the arrays, and ROI
    # volumes merge the left and right label of each structure.
