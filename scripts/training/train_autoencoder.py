...
# Channel indices of the tissue classes predicted by the segmentation teacher
# ('background', 'gray_matter', 'white_matter', 'ventricles', 'csf', 'deep_gray').
GM_INDEX, WM_INDEX = 1, 2


def gm_wm_logits(logits):
    """Reduce the 6-class tissue output to [other, GM, WM].

    Anatomical supervision is applied to gray and white matter only. The
    remaining classes are marginalised into a single 'other' class via
    logsumexp, which is the exact equivalent of summing their softmax
    probabilities, so the resulting 3-channel tensor keeps a valid
    distribution and the Dice term (with include_background=False) is
    computed over GM and WM alone.
    """
    rest = torch.cat([logits[:, :GM_INDEX], logits[:, WM_INDEX + 1:]], dim=1)
    other = torch.logsumexp(rest, dim=1, keepdim=True)
    return torch.cat([other,
                      logits[:, GM_INDEX:GM_INDEX + 1],
                      logits[:, WM_INDEX:WM_INDEX + 1]], dim=1)
    ...
    for epoch in range(args.n_epochs):
        ...
        for step, batch in progress_bar:
            ...
            with autocast(enabled=True):
                ...
                # restrict the supervision to gray and white matter
                seg_logits_pred = gm_wm_logits(seg_logits_pred)
                seg_logits_true = gm_wm_logits(seg_logits_true)
